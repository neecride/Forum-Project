            ...
            <div class="F_corps col-md-6">
              <div>
                <span>
                  ...
                  </a><?= !empty($posts->topic_lock) && $posts->topic_lock == 1 ? '&nbsp;<i style="font-size:11px;" class="fas fa-check-circle"></i>' : '' ; ?></span>
                </span>
              </div>
      ...
      <?php else: ?>
        <div class="card mb-2  child">
          <div class="card-body">
            ...
            <p><?= $Parsing->RenderText("_Vous pouvez en créer un si vous êtes connecter._") ?></p>
            <p><?= $Parsing->RenderText("**Toutefois souvenez-vous que ce site utilise du [markdown](https://fr.wikipedia.org/wiki/Markdown#Formatage)**"); ?></p>
            <p><?= $Parsing->RenderText("~~pourquoi~~"); ?></p>
            <p><?= $Parsing->RenderText("> parce que c'est cool"); ?></p>
          </div>
        </div>
      <?php endif; ?>  
      <!-- last -->
      ...
      <div class="page">
        <nav>
          <ul class="pagination mb-3 mt-3 pagination-sm">
            ...
            ?>
            <?php 
              $link = $router->generate('forum-tags', ['slug' => $params['slug'], 'id' => $params['id']]);
              ...
              for($i=1; $i <= $pages; $i++){
                if($i <= $nb || $i > $pages - $nb ||  ($i > $CurrentPage-$nb && $i < $CurrentPage+$nb)){
                  if($i == $CurrentPage) {
                    ...
                    echo '<li class="page-item"><a class="page-link" href='.$router->generate('forum-tags', ['slug' => $params['slug'], 'id' => $params['id']]).'>'. $i .'</a></li>' ;
                  }else{
                    echo '<li class="page-item"><a class="page-link" href='.$router->generate('forum-tags', ['slug' => $params['slug'], 'id' => $params['id']]).'?page='.$i.'>'. $i .'</a></li>' ;
                  }
                }else{
                  ...
                  echo '<li class="page-item"><a class="page-link" href='.$router->generate('forum-tags', ['slug' => $params['slug'], 'id' => $params['id']]).'?page='.($i-1) .'>...</a></li>';
                }
              }
            ?>
            <?php if($CurrentPage < $pages): ?>
              <li class="page-item">
                <a class="page-link" href="<?= $router->generate('forum-tags', ['slug' => $params['slug'], 'id' => $params['id']]) ?>?page=<?= $CurrentPage+1 ?>">
