...
Class TopicAction{
	...
	public function editBtnTopic()
	{
		if(isset($_SESSION['auth'])):
			...
			return "<a class=\"p-2\" href='$link'>
					<i class=\"far fa-edit\"></i><small class=\"text-muted\">&nbsp;&nbsp;Editer</small>
		  			</a>"; 
			endif;
		endif;
	}
	 ...
	 * @return void
	 */
	public function editBtnResponse(int $userAthor, int $auth,int $responseID)
	{
		if(isset($_SESSION['auth'])):
	 ...
	 * @return void
	 */
	public function resolvedStatusBtn()
	{
		if(isset($_SESSION['auth'])):
			...
			$link1 = $this->router->routeGenerate('lock',['id' => $this->firstTopic()->topicsid, 'lock' => 1, 'getcsrf' =>  $this->session->csrf()]);	
				if ($this->firstTopic()->topic_lock === 0):
					return "<a class=\"p-2\" href=\"$link1\">
					<i class=\"fas fa-lock\"></i><small class=\"text-muted\">&nbsp;&nbsp;Passé en résolu</small>
				  	</a>";
				elseif ($this->firstTopic()->topic_lock === 1):
					return "<a class=\"p-2\" href=\"$link0\">
					<i class=\"fas fa-check-circle\"></i><small class=\"text-muted\">&nbsp;&nbsp;Résolu</small>
				  	</a>";
				endif;
			endif;
		endif;
	 ...
	 * @return void
	 */
	public function stickyStatusBtn()
	{
		if(isset($_SESSION['auth'])):
			...
			if($_SESSION["auth"]->authorization > 1):
				if ($_SESSION['auth']->id == $this->firstTopic()->usersid || ($_SESSION["auth"]->authorization === 2 && $this->firstTopic()->authorization != 3)):
					if ($this->firstTopic()->sticky === 0):
						return "<a class=\"p-2\" href=\"$link1\">
						<i class=\"fas fa-thumbtack\"></i><small class=\"text-muted\">&nbsp;&nbsp;Mettre en sticky</small>
						</a>";
					elseif ($this->firstTopic()->sticky === 1):
						return "<a class=\"p-2\" href=\"$link0\">
						<i class=\"fas fa-ban\"></i><small class=\"text-muted\">&nbsp;&nbsp;Retiré le sticky</small>
						</a>";
					endif;
