...
class ForumAction{
	...
	/**
	 * choicesTagsSelected met l'attribue html selected
	 *
	 * @param  mixed $id
	 * @return mixed
	 */
	public function choicesTagsSelected($id)
	{
		foreach($this->Tags($this->router->matchRoute()['params']['id']) as $tags):
		    if(!empty($id == $tags->tag_id))
			{
				return 'selected';
			}
		endforeach;
	...
	public function accountLastTopic()
	{
		...
		ORDER BY Lastdate DESC LIMIT 5
	 ...
	 * @return mixed
	 */
	public function hot(int $id,int $limit = 10)
	{
		if($id >= $limit):
	 ...
	 * @return mixed
	 */
	public function isNew($readLast,$lastDate)
	{
		if(isset($_SESSION['auth'])):
			if(isset($readLast, $lastDate) && $readLast > $lastDate):
				return null;
			else:
				return '<div class="new" data-toggle="tooltip" data-placement="right" title="Sujet non lu"></div>';
			endif;
		endif;
	}
